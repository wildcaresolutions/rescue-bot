...
**Copilot says "I could not complete that response".** The `onError` handler in `agent.ts` surfaces the underlying error in the streamed payload. Check Workers logs for `[agent] streamText error:` lines — usually a rate-limit on the BYOK provider or a tool throwing.
