...
window.addEventListener('message', (event) => {
  ...
  const { theme, size, position, customCSS, autoOpen, buttonText, welcomeMessage, headerText } = event.data
...
async function initWidget() {
  ...
  if (_tenantSlug) {
    ...
    if (rc && !rc.platform) {
      ...
      // Header title: prefer widget_theme.headerText (operator-set), fall back
      // to the tenant's `name` (legacy). This is what renders in the widget
      // header bar — distinct from `name` which is also used in emails,
      // marketing copy, etc. Per-tenant override lets WildCare display
      // "WildCare's Emergency Wildlife Advice" while keeping name="WildCare".
      const wt = rc.widget_theme || {}
      widgetConfig.agentName = wt.headerText || rc.name || widgetConfig.agentName
      // Welcome message is used as the input placeholder ONLY. We do not
      // render it as a chat-bubble system message — the operator's content
      // is a prompt for the citizen, not a greeting from the bot.
  ...
  try {
    ...
    if (existingSessionId) {
      try {
        ...
        // Welcome text lives in the input placeholder only — never as a
        // chat bubble. A fresh/empty session shows an empty messages pane
        // plus the placeholder hint in the composer.
