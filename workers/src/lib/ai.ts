...
export const DEFAULT_GOOGLE_AI_GATEWAY_BYOK_ALIAS = ''
export const DEFAULT_OPENAI_AI_GATEWAY_BYOK_ALIAS = ''
export const DEFAULT_ANTHROPIC_AI_GATEWAY_BYOK_ALIAS = ''
...
async function runGatewayRequest(env: Env, request: GatewayRunRequest): Promise<Record<string, any>> {
  ...
  if (token) {
    ...
    if (request.byokAlias) headers.set('cf-aig-byok-alias', request.byokAlias)
...
function getByokAlias(env: Env, provider: 'google-ai-studio' | 'openai' | 'anthropic'): string {
  if (provider === 'google-ai-studio') {
    return clean(env.AI_GATEWAY_GOOGLE_BYOK_ALIAS) || DEFAULT_GOOGLE_AI_GATEWAY_BYOK_ALIAS
  }
  if (provider === 'openai') {
    return clean(env.AI_GATEWAY_OPENAI_BYOK_ALIAS) || DEFAULT_OPENAI_AI_GATEWAY_BYOK_ALIAS
  }
  return clean(env.AI_GATEWAY_ANTHROPIC_BYOK_ALIAS) || DEFAULT_ANTHROPIC_AI_GATEWAY_BYOK_ALIAS
}
...
export async function runGatewayChatText(opts: {
  ...
  if (opts.model.startsWith('openai/') || opts.model.startsWith('anthropic/') || opts.model.startsWith('google/') || opts.model.startsWith('workers-ai/')) {
    ...
    result = await runGatewayRequest(opts.env, {
      ...
      byokAlias: getByokAlias(opts.env, 'google-ai-studio'),
      query: {
