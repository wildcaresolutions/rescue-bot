...
agentApp.post('/admin/agent', async (c) => {
  ...
  const { anthropic, byokAlias, usingDirectKey } = provider
  ...
  const result = streamText({
      ...
      onError: (event) => {
        ...
        // response" while the real error (auth, rate limit, BYOK misconfig)
        // was invisible to anyone not tailing Workers logs.
        const err = event.error
        const msg = err instanceof Error ? err.message : String(err)
        console.error('[agent] streamText error:', msg, {
          model: AGENT_MODEL,
          byokAlias: byokAlias || '(none)',
          usingDirectKey,
